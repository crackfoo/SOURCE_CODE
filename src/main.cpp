...
#if defined(NDEBUG)
# error "litecoinextreme cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xafaa9aacf8d378b84a7e9433f0a4c0b6b4f03a0ff19b716fefa9b17f93c0d5a8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // litecoinextreme: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "litecoinextreme Signed Message:\n";
...
{
    // litecoinextreme: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1440 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // litecoinextreme: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // litecoinextreme: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1521530127, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Zuckerberg loses 6 billion in 1 day amid data controversy";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c89c1858acedead1cb07b64c16fad42f17db49688068209d39767238d6dd16d855e2f6a43f3b2851c0cb459cd45e7ad3a8f311817a7723aad77d33d1c30d9d6f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521530127;
        ...
        assert(block.hashMerkleRoot == uint256("0x9514eef935e205f91a1d1709a5233a5f64c01a18ebfec803ebbcc45629edf8e1"));
...
unsigned char pchMessageStart[4] = { 0xd3, 0x90, 0x75, 0x82 }; // litecoinextreme: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("litecoinextremeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("litecoinextremeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static litecoinextremeMiner(CWallet *pwallet)
{
    printf("litecoinextremeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litecoinextreme-miner");
    ...
    try { loop {
        ...
        printf("Running litecoinextremeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("litecoinextremeMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&litecoinextremeMiner, pwallet));
}
